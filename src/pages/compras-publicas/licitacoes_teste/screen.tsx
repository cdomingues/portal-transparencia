...
function Screen() {
  ...
  const fetchData = async () => {
    ...
    try {
      ...
      setLicitacoes(response.data.results || []);
      setCurrentPage(1);
    } catch (error) {
  ...
  return (
    <ContainerBasic title={title} description={description}>
      ...
      <Box display='flex' flexDir='row' gap="8px"><Text fontWeight='bold' border='1px solid black' p='0'>DESCRIÇÃO:</Text>  <Text >   {row.descricao}</Text></Box>
