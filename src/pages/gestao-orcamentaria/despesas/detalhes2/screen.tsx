...
function Screen({exercicio_empenho, nr_empenho}: ScreenProps) {
 ...
 /*  const url = `https://dadosadm.mogidascruzes.sp.gov.br/api/despesa_publicidade?exercicio_empenho=${exercicio_empenho}&nr_empenho=${nr_empenho}`;
console.log('url:', url) */

  // Recupera a despesa do sessionStorage
useEffect(() => {
    const storedDespesa = sessionStorage.getItem("selectedDespesa");
    if (storedDespesa) {
      try {
        const parsed = JSON.parse(storedDespesa);
        setDespesa(parsed);
        console.log("Despesa carregada do sessionStorage:", parsed);
      } catch (err) {
        console.error("Erro ao parsear despesa do sessionStorage:", err);
      }
    }
  }, []);
  ...
  useEffect(() => {
    const fetchArquivoAdiantamentos = async () => {
      ...
      try {
        const response = await fetch(
         `https://dadosadm.mogidascruzes.sp.gov.br/api/arquivo_publicidade?nr_empenho=${despesa.nr_empenho}&exercicio_empenho=${exercicio_empenho}` 
            ...
            <Table variant="simple" size="md" width="100%" overflow="hidden" mb={5}>
              ...
              <Tbody>
                {[
                   ...
                   ["Arquivo",  <a
    key="arquivo-link"
    href={despesa.arquivo_despesa}
    target="_blank"
    rel="noopener noreferrer"
    style={{ color: "blue", textDecoration: "underline" }}
  >
    {despesa.arquivo_despesa ? "Download" : "Não disponível"}
  </a>]
