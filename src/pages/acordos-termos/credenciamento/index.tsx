...
function Controller({ ano, secretaria }: { ano?: string; secretaria?: string }) {
    ...
    try {
      ...
      while (hasMore) {
        const res = await axios.get("https://dadosadm.mogidascruzes.sp.gov.br/api/acordos", {
          params: {
            page,
            tipo: 11, // apenas tipo 11: Credenciamento
            ano, // opcional
      ...
      if (axios.isAxiosError(error) && error.response?.status === 404) {
        console.warn("API retornou 404 para o tipo 11. Definindo dados como vazios.");
        setData([]);
      } else {
