...
function Screen({ handler: {  data, loading } }: PropsInput) {
  ...
  const [termo,setTermo] = useState<any>(null);
    ...
    const [selectedYear, setSelectedYear] = useState<number | undefined>(undefined); // Estado para o ano selecionado (inicia sem filtro)
...
(item.contratada || "").toLowerCase().includes(searchTerm.toLowerCase())
    ...
    const exportToJSON = (data: any) => {
      ...
      link.setAttribute("download", "dados_credenciamento.json");
    ...
    const years = [...new Set(data.map((item) => (item.tc).split('/')[1]))].sort((a, b: any) => b - a);
  
    // Nota: O ID da página para o hook usePagina pode precisar ser configurado no CMS.
    // Usando um ID genérico ou baseado nos outros (31, 32...).
    // Se não houver conteúdo dinâmico, o título virá por padrão.
    const {paginaData, loadings} = usePagina("81") as {
      paginaData: { titulo: string; descricao: string; conteudo: string } | null;
      loadings: boolean;
      error: any;
    };

    const titlePage = paginaData?.titulo ?? "Credenciamento";
    const description = paginaData?.descricao ?? "Lista de credenciamentos da administração municipal";
  ...
  return (
    ...
    >
      <CsvDownload
        filename={"dados_credenciamento.csv"}
        data={data}
        style={{
          width: "100%",
          height: "100%",
