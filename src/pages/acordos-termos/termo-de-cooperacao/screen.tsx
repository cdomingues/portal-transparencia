...
function Screen({ handler: {  data, loading } }: PropsInput) {
  ...
  const [termo,setTermo] = useState<any>(null);
    ...
    const [selectedYear, setSelectedYear] = useState<number | undefined>(); // Estado para o ano selecionado
  ...
  const filteredContratos = data.filter((item) => {
      if (selectedYear) {
        return Number(item.data_inicio.split('/')[2]) === selectedYear; // Filtra pelo campo "ano"
      }
...
(item.contratada || "").toLowerCase().includes(searchTerm.toLowerCase())
    ...
    const years = [...new Set(data.map(item => item.data_inicio.split('/')[2]))].sort((a, b) => b - a);
