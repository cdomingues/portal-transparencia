...
import ItensEmpenhoTexto from "../../../components/ItemEmpenho";
...
function Screen({
  ...
  const [expandedEmpenhos, setExpandedEmpenhos] = useState<Record<string, boolean>>({});
  const [itensEmpenho, setItensEmpenho] = useState<Record<string, ItemEmpenho[]>>({});
  ...
  .filter((item) =>
    ...
    item.id_empenho?.toLowerCase().includes(searchTerm.toLowerCase())  // Evita erro quando item.vinculo for null
  );
...
console.log(years)
  ...
  .sort((a, b) => Number(b.nr_empenho) - Number(a.nr_empenho))
  .map((row) => (
    <Box
      key={row.id}
    ...
    >
      ...
      <Text><strong>Item: </strong><ItensEmpenhoTexto nr_empenho={row.nr_empenho} exercicio_empenho={row.exercicio_empenho} /></Text>
    </Box>
  ))}
