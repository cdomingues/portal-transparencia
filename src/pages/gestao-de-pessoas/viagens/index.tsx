...
const Despesas = () => {
  ...
  const { paginaData, loadings, error } = usePagina("89");
  const subelementos = ['1 - PASSAGENS PARA O PAÍS                             ', '2 - PASSAGENS PARA O EXTERIOR                         ', '73 - TRANSPORTE DE SERVIDORES / EMPREGADOS             ','74 - FRETES E TRANSPORTES DE ENCOMENDAS                ', '80 - HOSPEDAGENS                                       ' ];
  ...
  try {
      ...
      while (hasMore) {
        const res = await axios.get('https://dadosadm.mogidascruzes.sp.gov.br/api/despesas?', {
          params: {
            ...
            subelemento: sub,
            //elemento: '3.3.90.33.00 - PASSAGENS E DESPESAS COM LOCOMOÇÃO',
          },
        });
    ...
    const ordenado = allResults.sort(
      (a, b) => Number(a.id_empenho) - Number(b.id_empenho)
    );
  ...
  try {
    const responses = await Promise.all(
      subelementos.map((sub) =>
        axios.get('https://dadosadm.mogidascruzes.sp.gov.br/api/despesas', {
          params: {
            page: pagina,
            exercicio_empenho: ano,
            subelemento: sub,
    ...
    let resultados: Despesa[] = [];
    let totalCount = 0;

    responses.forEach((res) => {
      resultados = [...resultados, ...res.data.results];
    ...
    setTotalPaginas(Math.ceil(totalCount / 50));
  } catch (error) {
