...
function Screen() {
  ...
  const {paginaData, loadings, error} = usePagina("88");
...
{conteudo && (
  ...
  />
      ...
      <Text fontSize={accessibility?.fonts?.regular} mb="10px" ml="5px">
        Última atualização: <strong>05/03/2026</strong>
      </Text>
        ...
        <>
          <Table overflowX="auto" width="100%">
            ...
            <Tbody fontSize="12px">
              {paginatedData
              .filter( (row) =>
                !["Adiantamento de Férias", "Adiantamento Salarial"].includes(row.tipo_folha))
             .map((row, index) => (
                <Tr
                  key={index}
                  bg={
