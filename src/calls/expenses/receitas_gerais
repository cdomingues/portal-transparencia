...
export const getReceitaEmenda = async (years?: number) => {
  try {
    let nextPage = "https://dadosadm.mogidascruzes.sp.gov.br/api/lista_receitas";
    let allResults: any[] = [];
    let allYears: number[] = [];
...
publicacaoFim: moment(row.publicacaoFim).format("DD/MM/YYYY"),
    ...
    return { revenues: mappingRows }; // Remove duplicatas
  } catch (error) {
